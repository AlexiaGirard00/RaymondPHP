<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (isset($_POST["form_submit"])) {
            ...
            // verifier si case est cocher pour effacer = delete l'entré via ID
...
// TODO: MOdifier query pour insert table imageproduit
        ...
        if (empty($nom_err) && empty($desc_err) && empty($dim_err) && empty($typeBois_err) && empty($prix_err) && empty($image_err)) {
            $sql = "UPDATE `dbo.produits` SET NomProduit = :nom, IdCategorieFk = :cat, DescriptionProduit = :desc, DimensionProduit = :dim, " . 
                " TypeDeBoisProduit = :typeBois, CapaciteProduit = :capacite, PrixProduit = :prix, ImageChemin = :image, " . 
    ...
    <div class="container my-5">
        ...
        <form action="<?php echo htmlspecialchars(($_SERVER["PHP_SELF"])); ?>" method="post">
            ...
            <div class="form-group">
                ...
                <input  id="image1" disabled type="file" name="upload_image[]" class="form-control" required>
            </div>
            ...
            <div class="form-group">
                ...
                while ($rowImage = mysqli_fetch_array($resImage)) {?>
                  ...
                  '</div>'+
                  '<div>'+
                  '<input type="checkbox" id="" onclick="activerFichier(<?php echo $numero?>)" name="image" value="<?php echo $rowImage['IdImageProduit'];?>" >'+
                    '<label for="image" style="padding-left:1%;"> Cocher pour effacer</label><br>' +
                  '</div>';
