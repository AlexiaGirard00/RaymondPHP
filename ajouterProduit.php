    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (isset($_POST['form_submit'])) { 
            ...
            if (!empty($fileNames)){                 
                ...
                // Message erreur
                $errorUpload = !empty($errorUpload)?'Erreur: '.trim($errorUpload, ' | '):''; 
                $errorUploadType = !empty($errorUploadType)?'Erreur type de fichier: '.trim($errorUploadType, ' | '):''; 
                ...
                if(!empty($insertValuesSQL)){ 
                    ...
                    if($insert){ 
                        $statusMsg = "Les fichiers sont téléchargés avec succès.".$errorMsg; 
                    }else{ 
                        $statusMsg = "Désolé, une erreur s'est produite lors du téléchargement de votre fichier."; 
                    } 
                }else{ 
                    $statusMsg = "Un problème est survenu.".$errorMsg; 
                } 
            }else{ 
...
<!--Fermeture div en lien avec menuAdmin-->
</div>
</div>    
